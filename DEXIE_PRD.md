...
- introducing a “DB lifecycle” provider that initializes Dexie only after reading the saved cloud configuration and can _hot-reinitialize_ the DB instance when config changes (without a full page reload)
...
- Ensure Dexie Cloud is configured before opening and querying the DB:
    - Read saved config (Dexie Cloud database URL, any flags) _before_ creating/using the Dexie instance.
...
- In SW file:
    - precacheAndRoute(\_\_WB_MANIFEST)
