...
export const WindowProvider: React.FC<WindowProviderProps> = ({ children }) => {
    ...
    const persistWindowStates = useCallback((windowsToSave: WindowState[]) => {
        ...
        storageService.set(KV_KEYS.openWindows, windowsToSave.map(w => w.appId)).catch(() => undefined);
    }, []);

    // Restore session
    useEffect(() => {
        if (sessionRestoredRef.current || !isInitialized || apps.length === 0 || openAppIdsRef.current.length === 0) return;
            ...
            setTimeout(() => {
                ...
                setWindows(prev => {
                    ...
                    const newWindow: WindowState = {
                        ...
                        size: savedState?.state?.size ?? { width: app.defaultWidth ?? 800, height: app.defaultHeight ?? 600 }
                    };
    ...
    const openWindow = useCallback((appId: string, contentProps?: Record<string, unknown>) => {
    ...
    const toggleMaximizeWindow = useCallback((id: string) => {
        setWindows(prev => {
            const updated = prev.map(w => w.id === id ? { ...w, isMaximized: !w.isMaximized, zIndex: getNextZIndex() } : w);
